...
- **Evening, tree `b8d24a9`: the ten silhouettes ran, 10 of 10 passed** (`wdi-ab`, `@silhouettes`, no texture warning, every body type read back). The images are still not judgeable on the adults: the bracket, the name label and the hover tooltip cover the waist. **On both children the capture shows a light-blue shell around the whole child body, up to the hair, and a pair of light-blue trousers on the ground beside the pawn.** If that is the trousers drawn as a sack on a real child body, it is a defect of the mod that no earlier run could see, because every earlier child was an adult body in child trousers. Suspicion only, until a capture without overlays. The framing was changed (pawn not selected, pointer moved onto the Architect button) and is to be tried on two scenarios.
